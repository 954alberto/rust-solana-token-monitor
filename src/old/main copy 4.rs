...
fn extract_mint_events(block: UiConfirmedBlock) -> Result<(), Box<dyn std::error::Error>> {
    ...
    if let Some(transactions) = block.transactions {
        //println!("Number of transactions in the block: {}", transactions.len());
        
        for (index, tx) in transactions.iter().enumerate() {
            println!("Processing transaction index: {}", index); // Debugging output
            // println!("Processing transaction {}: {:?}", index, tx); // Debugging output

            // Access the encoded transaction directly
            ...
            if let Some(ui_transaction) = encoded_transaction.decode() {
                // Print the decoded transaction for debugging
                //println!("Decoded transaction: {:?}", ui_transaction);
                ...
                // Access the instructions (if needed)
                let instructions = ui_transaction.message.instructions();
                for instruction in instructions {
                    // You can process the instruction here
                    //println!("Instruction: {:?}", instruction);
                }
                let signatures = ui_transaction.signatures;
                for signature in signatures {
                    // You can process the instruction here
                    println!("Signature: {:?}", signature);
                }                
            } else {
                eprintln!("Failed to decode transaction for transaction index: {}", index);
            }
        }
    } else {
